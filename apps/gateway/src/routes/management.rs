...
pub async fn revoke_key(
    ...
    match async {
        ...
        // Read before revoking/deleting: the hash is needed to invalidate the caches.
        ...
        // If the key is already revoked or caller explicitly requested permanent deletion:
        if key.revoked_at.is_some() || permanent {
            repo::delete_api_key(pool, context.org_id, key_id).await?;
            auth::invalidate_key(state.store.as_ref(), &state.key_cache, &key.key_hash).await?;
...
pub async fn update_member_role(
    ...
    match async {
        ...
        Ok::<_, AegisError>(respond(
            StatusCode::OK,
            serde_json::json!({"updated": true, "role": request.role}),
        ))
    }
    .await
    {
