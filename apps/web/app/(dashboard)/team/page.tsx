...
export default function TeamPage() {
  ...
  async function handleRoleChange(member: Member, newRole: string) {
    ...
    try {
      await api.updateMemberRole(member.user_id, newRole);
      setNotice(`Updated ${member.email}'s role to ${newRole}.`);
      await load();
    } catch (caught) {
      setError(
        caught instanceof ApiError ? caught.message : "Could not update member role.",
      );
    }
  }
                  ...
                  <Td>
                    {member.role === "owner" ? (
                      ...
                      <select
                        ...
                        className="rounded-lg border border-[var(--color-desk-line)] bg-[var(--color-surface2)] px-2.5 py-1 text-xs font-bold text-[var(--color-ink)] hover:border-[var(--color-accent)] focus:outline-none transition-colors cursor-pointer"
                      >
