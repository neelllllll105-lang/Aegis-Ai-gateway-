        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Stat
              ...
              sublabel={`after Aegis ${((org?.organization.savings_share_bp ?? 0) / 100).toFixed(0)}% performance fee`}
              accent
            />
            <Stat
              label="Actual Spend"
              value={formatUsdCompact(summary?.actual_cost_mc ?? 0)}
              sublabel={`vs ${formatUsd(summary?.baseline_cost_mc ?? 0)} unoptimized baseline`}
            />
            <Stat
              label="Total Requests"
              value={formatCount(summary?.requests ?? 0)}
              sublabel={`${formatPercent(derived?.cache_hit_rate ?? 0)} served via 0ms cache`}
            />
            <Stat
              label="Cost Reduction"
              value={formatPercent(derived?.savings_percent ?? 0)}
              sublabel="total budget saved"
            />
          </div>
          ...
          <Card className="mt-6 p-6">
            <h3 className="text-sm font-medium text-[var(--color-ink)]">
              Financial Optimization & Savings Ledger
            </h3>
            <p className="mt-1 text-xs text-[var(--color-muted-light)]">
              Verifiable accounting of cost reduction across semantic caching, intelligent model routing, and prompt compression.
            </p>

            <dl className="mt-5 space-y-3">
              <LedgerRow
                label="Unoptimized Baseline Cost (what requests would cost on requested frontier models)"
                value={formatUsd(summary?.baseline_cost_mc ?? 0)}
              />
              <LedgerRow
                label="Actual Provider Spend (direct upstream charges on your BYOK keys)"
                value={`− ${formatUsd(summary?.actual_cost_mc ?? 0)}`}
              />
              <div className="border-t border-[var(--color-line)] pt-3">
                <LedgerRow
                  label="Gross Savings Avoided (total optimization delta)"
                  value={formatUsd(summary?.gross_savings_mc ?? 0)}
                  emphasis
                />
              </div>
              <LedgerRow
                label={`Aegis Software Fee (${((org?.organization.savings_share_bp ?? 0) / 100).toFixed(0)}% performance share)`}
                value={`− ${formatUsd(summary?.aegis_fee_mc ?? 0)}`}
              />
              <div className="border-t border-[var(--color-line)] pt-3">
                <LedgerRow
                  label="Your Net Bottom-Line Savings"
